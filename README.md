...
| 데이터 소스 | 샘플 수 | 설명 |
|-------------|---------|------|
| 수집 캡션 | 377 | Instagram에서 수집한 한국어 라이프스타일 캡션 |
...
**업샘플링 전략**: 페르소나 데이터를 **3배 업샘플링**하여 수집 데이터 대비 비중 확보
...
```bash
# Instagram 수집 (로그인 필요 - .env에 계정 설정)
python3 scripts/crawl_instagram.py --headless --count 30

# 순환 파이프라인 (수집 → 데이터 변환 → 키워드 추출 → 학습)
python3 scripts/run_cycle.py --skip-crawl  # 수집 건너뛰고 기존 데이터로
```
